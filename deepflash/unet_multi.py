...
class Unet2D:
    ...
    def predict(self, tile_generator):
        ...
        for tileIdx in tqdm(range(tile_generator.__len__())):
            ...
            #pdb.set_trace()
            for i in range(len(softmax_score)):
                if len(smscores[i]) < outIdx + 1:
                    smscores[i].append(np.empty((*outShape, self.n_classes)))
                    segmentations[i].append(np.empty(outShape))
                smscores[i][outIdx][outSlice] = softmax_score[i][0][inSlice]
                segmentations[i][outIdx][outSlice] = np.argmax(softmax_score[i][0], axis=-1)[inSlice]
