...
class DataAugmentationGenerator(keras.utils.Sequence):
  ...
  """
    ...
    def __data_generation(self, indexes):
        ...
        Y = np.zeros((self.batch_size, *self.output_shape, self.n_coders), dtype=np.int)
        W = np.empty((self.batch_size, *self.output_shape, self.n_coders), dtype=np.float)
        for i, idx in enumerate(indexes):
            ...
            for j in range(self.n_coders):
                Y[i, ..., j] = self.deformationField.apply(self.labels[idx][...,j], center, self.padding, 0)
                W[i, ..., j] = self.deformationField.apply(self.weights[idx][...,j], center, self.padding, 1)

        return (
            {
                "data": X,
                "labels": np.moveaxis(keras.utils.to_categorical(Y, num_classes=self.n_classes), 3, -1),
                "weights": W,
            },
            None,
        )
